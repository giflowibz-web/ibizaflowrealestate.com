...
// Parse multi-line ANTHROPIC_CUSTOM_HEADERS from env (Orchids proxy auth)
...
const orchidsKey = customHeaders['x-orchids-api-key'] || process.env.ANTHROPIC_AUTH_TOKEN || process.env.ANTHROPIC_API_KEY || ''
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { title_es, area, bedrooms, bathrooms, size_built, size_plot, features, listing_type, price, property_type } = body

    const propertyTitle = title_es || 'Propiedad en Ibiza'
    const featuresList = Array.isArray(features) && features.length > 0 ? features.join(', ') : 'no especificadas'
...
FORMATO DE RESPUESTA (exactamente así, sin markdown):
...
SEO_FR: [mots-clés séparés par des virgules]`
    ...
    return NextResponse.json({
      title_es: titleEsMatch?.[1].trim() ?? '',
      title_en: titleEnMatch?.[1].trim() ?? '',
      title_fr: titleFrMatch?.[1].trim() ?? '',
      description_es: esMatch?.[1].trim() ?? '',
      description_en: enMatch?.[1].trim() ?? '',
      description_fr: frMatch?.[1].trim() ?? '',
      seo_keywords_es: seoEsMatch?.[1].trim() ?? seoKeywords_es,
      seo_keywords_en: seoEnMatch?.[1].trim() ?? seoKeywords_en,
      seo_keywords_fr: seoFrMatch?.[1].trim() ?? seoKeywords_fr,
    })
  } catch (err: unknown) {
    console.error('Generate description error:', err)
    return NextResponse.json({ error: err instanceof Error ? err.message : 'Error generating' }, { status: 500 })
  }
}
